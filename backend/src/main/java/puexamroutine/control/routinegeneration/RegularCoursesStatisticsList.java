...
public class RegularCoursesStatisticsList {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(RegularCoursesStatisticsList.class);
    ...
    public final int getMaxExtremeCourseDisplacement() {
        if (this.RegularCoursesWithMaxDisplacement == null) {
            this.calculateRegularCoursesWithMaxDisplacement();
        }
    ...
    public final double calculateMinIndexVariance() {
        ...
        LOGGER.debug("first:{} sec:{} sec1:{} var:{}", first, sec, sec1, this.MidIndexVariance);
        return this.MidIndexVariance;
    }

    public final double getDisplacementVariance() {
        if (this.DisplacementVariance == java.lang.Double.MIN_VALUE) {
            this.calculateDisplacementVariance();
        }
        return this.DisplacementVariance;
    }
    ...
    public final double getMeanDisplacement() {
        if (this.MeanDisplacement == java.lang.Double.MIN_VALUE) {
            this.calculateMeanDisplacement();
        }
        return this.MeanDisplacement;
    }
    ...
    public final double getAverageIndexVariance() {
        if (this.MidIndexVariance == java.lang.Double.MIN_VALUE) {
            this.calculateMinIndexVariance();
        }
        return this.MidIndexVariance;
    }

    public double getOptimizeValue() {
        return this.getMaxExtremeCourseDisplacement() * this.getMeanDisplacement() * this.getTotalCourses();
    }
    ...
    public int getMinimumIndexOfList() {
        if (this.Min_Index == java.lang.Integer.MIN_VALUE) {
            this.calculateMinIndexAmongList();
        }
        return this.Min_Index;
    }

    public int getMaximumIndexOfList() {
        if (this.Max_Index == java.lang.Integer.MIN_VALUE) {
            this.calculateMaxIndexAmongList();
        }
        return this.Max_Index;
    }

    void print(String... t) {
        LOGGER.debug("This is printing in variance");
        for (int i = 0; i < t.length; i += 2) {
            LOGGER.debug("{}::{}", t[i], t[i + 1]);
        }
    }
